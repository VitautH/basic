...
<div class="container" style="padding: 0;">
    <div class="offer">
        <div>
            <span class="title_1"><?= _t('Хотите воспользоваться услугой?')?></span>
        </div>
        <div>
                <div class="arrow_box"><a href="/<?=\Yii::$app->language;?>/products"><?= _t('Забронировать сейчас')?></a></div>
        </div>
    </div>
</div>
<article>
...
<div class="row">
    <div class="banner_left col-lg-3 col-xs-12 col-sm-5">
            <?php if ($banner !== null) {
                ?>
                <a href="<?= $banner->url; ?>"> <img width="270" height="618"
    ...
    <div class="content col-lg-9 col-xs-12 col-sm-7">
        ...
        <? foreach ($articles as $key=>$article) : ?>
            <div class="<?= ($key== 2) ? 'end':'' ?> <?= ($key== 1) ?'news-middle':''?> news col-lg-4 col-xs-12">
                <h2>
                    <?= $article->title?>
                </h2>
                <p>
                ...
                <span class="more">     <a href="/<?=\Yii::$app->language;?>/articles/view?id=<?= $article->id;?>"> <?= _t('Подробнее')?> </a> </span>
            </div>
        <? endforeach; ?>
        </div>
...
<h2><?= _t('Наши партнёры')?></h2>
   <?
   $sliders_img =[];

   foreach ($casinos_slider_img as  $key=>$casino) {
$img_url = Yii::$app->imagemanager->getImagePath($casino->logo_id, '118', '121');
       array_push($sliders_img,   '<a href="/'.\Yii::$app->language.'/casino/view?id='.$casino->id.'">'.Html::img($img_url, $options = ['style' => [
               'width' => '118px',
               'height' => '121px',
               'border-radius'=>'120px'
           ],]).'</a>');
...
<div class="row">
    <div class="bonus_plan_left_sidebar_block col-lg-4 col-xs-12">
        <h3>
<?= _t('Популярные бонус-планы')?>
        </h3>
        <div class="line-bonus-plan"></div>
    </div>
...
<div class="container bottom">
    <div class="row">
        <div class="left_block col-lg-4 col-xs-12">
            <h3>
<?= _t('Приемущества раннего бронирования')?>
            </h3>
        </div>
